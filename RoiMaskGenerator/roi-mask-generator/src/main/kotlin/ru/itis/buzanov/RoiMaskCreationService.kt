...
import ru.itis.buzanov.model.Building
import ru.itis.buzanov.model.BuildingType
import ru.itis.buzanov.util.Util
...
class RoiMaskCreationService {
    ...
    fun init() {
        ...
        val result = File("D:\\BuildingClassificator\\screencast\\result")
        val map = readBuildingsCsv(result)
        ...
        map.entries.forEach { entry ->
            ...
            buildings.forEachIndexed { _, it ->
                ...
                val file =
                    File("D:\\BuildingClassificator\\screencast\\masks\\RGB-PanSharpen_${imageName}_${it.id}.png")
                if (!file.exists()) file.createNewFile()
                println("$imageName building N-${it.id} " + Imgcodecs.imwrite(file.absolutePath, mask))
            }
        }
            ...
            while (reader.readLine().also { line = it } != null) {
                ...
                while (p1 >= 0) {
                    ...
                    for (i in pts.indices) {
                        ...
                        for (element in pt) {
                            val coord = element.trim().split(" ").toTypedArray()
